...
class Robot:
    ...
    def __init__(self, state=0):
        ...
        self.K1 = 5
        # stage2 system
        self.following_linear = 0.2
        self.diff1 = Differentiator()
        self.dd = 0.2
        self.Kp = -1.2 #-1.5
        self.Kd = -0.7
        self.aggresiveness = 3.0 #1.5
        self.angle_detect_dist = 0.3
        self.angle_detect_thres = np.pi/16
        self.err_smoother = Smoother(4)
    ...
    def calculate_action(self):
        ...
        if self.state == 0: #find wall
            ...
            #print 'Stage0: ang = %f \t lin = %f' % (msg.angular.z,msg.linear.x)

            #check wall positioning condition
            if(np.abs(e_lin) < 0.06 and np.abs(e_ang) < 0.05):
                msg.linear.x = 0
                msg.angular.z = 0
        ...
        # STATE 1
           ...
           if(self.sonar['front'] > 0.4 and np.abs(self.sonar['left']-self.dd) < 0.05):
        ...
        # STATE 2
            ...
            mu, theta = calc_distance_orientation(self.sonar['left'], self.sonar['fleft'])
	...
	elif self.state == 4: #halt
			msg.angular.z = 0.0
			msg.linear.x = 0.0
	elif self.state == 5: #forward drive
		msg.linear.x = 1.0
		msg.angular.z = 0.0
        else:
