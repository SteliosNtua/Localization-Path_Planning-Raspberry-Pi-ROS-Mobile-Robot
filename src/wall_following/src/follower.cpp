...
st = 1 ;                           /* Η κατάσταση στην οποία βρίσκεται το ρομπότ ως προς τα εμπόδια */

  while (ros::ok()){
    geometry_msgs::Twist velocity;
    /* Συνθήκη για μηδενισμό ταχυτήτων και σταμάτημα του ρομπότ */
      ...
      /**		v_d = 0.2;
		...
		velocity.angular.z = a_d; 
    		...
    		a = atan (((sonarFL_val + 0.02)  * 0.866025403 - (sonarL_val + 0.04)) / ((sonarFL_val + 0.02)  * 0.5));    /* Υπολογισμός της γωνίας α */
    		error = - cos(a) * sonarL_val + 0.2;       /* Υπολογισμός του σφάλματος */
	/**	if (sonarF_val <= 0.29 || sonarFR_val <=0.29) { /* Συνθήκη όπου έχουμε έρθει πολύ κοντά στον τοίχο (το 0.29 ορίζουμε 									απόσταση ασφαλείας) και αν συνεχίσουμε θα συγκρουστούμε οπότε πρέπει να 									στρίψει (2ο state) */
      ...
      /**			v_d = 2.4 * tf * tf - 3.2 * tf * tf *tf;
			...
			else v_d = 0.6;                         /* Φάση σταθερής ταχύτητας */
    		velocity.linear.x = 0.2;
		  		velocity.angular.z = - (10 * (error) + 3 * (error - prev_error));           /**** Υλοποίση του PD ελεγκτη που κρατάει 													σταθερή κάθετη απόσταση (από αριστερά) του 													ρομπότ με τον τοίχο 0.3 ****/
      /**		velocity.linear.x = v_d;
